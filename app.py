...
if st.button("📄 สร้างไฟล์ PDF และดาวน์โหลด", type="primary"):
    if not customer_name:
        st.error("กรุณาใส่ชื่อลูกค้าก่อนสร้าง PDF ครับ")
    else:
        buffer = io.BytesIO()
        c = canvas.Canvas(buffer, pagesize=A4)
        
        # ลงทะเบียนฟอนต์ (ต้องมีไฟล์ Sarabun-Regular.ttf ใน GitHub)
        try:
        ...
        # วาดพื้นหลัง (ต้องมีไฟล์ template.jpg ใน GitHub)
        ...
        # พิกัดเดียวกับที่คุณ Mu ใช้ในคอมพิวเตอร์
        ...
        st.success("สร้างไฟล์สำเร็จ! คุณสามารถแชร์ไฟล์นี้เข้า LINE ได้เลย")
